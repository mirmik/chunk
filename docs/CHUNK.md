...
CHUNK использует yaml-lite синтаксис потому что нейросети достаточно стабильно пишут yaml документы (в отличие от unified diff)
## Комментарии и символ `#`
yaml-lite, который использует CHUNK, **не поддерживает комментарии YAML**.
Символ `#` нигде не имеет специального смысла и всегда считается частью строки.
Для пояснений к патчу используй поле `description`, поле `comment` у операций или текст внутри `payload`.
...
Опционально можно добавить описание:
...
При поиске маркера мы **игнорируем различия в отступах, хвостовых пробелах и количестве пустых строк**, чтобы патчи не ломались из-за форматирования различных версий ide и вэбпросмотрщиков, в том числе используемых нейросетями при просмотре интернет ссылок.

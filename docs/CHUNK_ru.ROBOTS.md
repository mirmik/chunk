...
3. **Формат комментариев.**.
   Комментарии к патчу в целом и отдельным операциям выполняй с помощью специальных полей, `description` для патча в целом и `comment` для отдельной операции. Не используй `#` как yaml-комментарий для пояснений назначения операций патча. 
...
5. Нельзя использовать комментарии `#` внутри YAML-документа CHUNK.
6. Запрещается переставлять слова в естественном тексте.
7. Оставлять метки вроде `::contentReference[oaicite:0]{index=0}` в теле документа.

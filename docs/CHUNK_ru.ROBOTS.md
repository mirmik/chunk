...
3. **Формат комментариев.**.
   Комментарии к патчу в целом и отдельным операциям выполняй с помощью специальных полей, `description` для патча в целом и `comment` для отдельной операции. Также можешь использовать комментарии в соответствии со стандартными правилами YAML.
...
6. **Многострочные строки (`payload`, большие фрагменты кода).**
   ...
   * Для `marker` и `payload` **всегда** используй YAML-литерал `|`, даже если их значение однострочное.
...
Используются для работы с сущностями языков C++, Python и XML.
...
* `replace_py_block` — заменить Python-подобный блок, начинающийся с двоеточия и длящийся до выхода за предел идентации.
* `replace_xml_block` — заменить XML-блок по открывающему тегу до соответствующего закрывающего тега.
...
* `language` — основной язык. Поддерживаются `"python"`, `"c++"`.
...
### 4.3.3. `replace_xml_block`

Команда для XML-блоков по открывающему и закрывающему тегу.

* `marker` должен включать строку с открывающим тегом (например, `<item>` или `<item id="foo">`). При поиске маркера используются те же правила нормализации, что и для других команд.
* Область замены — от начала маркера до соответствующего закрывающего тега `</имя_тега>`, с учётом вложенных одноимённых тегов.
* Блок может быть самозакрывающимся (`<tag ... />`) — в этом случае заменяется только сам тег.
...
4. Всегда записывай `marker` и `payload` как многострочный блок. Даже если блок состоит из одной строки.

...
3. **Формат комментариев.**.
   Комментарии к патчу в целом и отдельным операциям выполняй с помощью специальных полей, `description` для патча в целом и `comment` для отдельной операции. Не используй `#` как yaml-комментарий. 
